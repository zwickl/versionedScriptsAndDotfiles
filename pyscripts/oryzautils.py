...
class Oryza:
    def __init__(self):
        self.taxon_names = [ 'O. sativaj AA', 'O. barthii AA', 'O. brachyantha FF', 'O. glaberrima AA', 'O. minuta BB', 'O. minuta CC', 'O. nivara AA', 'O. officinalis CC', 'O. punctata BB', 'O. rufipogon AA' ]

oryza = Oryza()

def extract_coords_for_seqs_in_alignment(filenames):
    '''This script is extracting coords from something like the following that I write to the end of the nexus alignments, and returning
    ...
    #first get the coordinates for each gene at each locus 
    for filename in filenames:
        ...
        coreFilname = extract_core_filename(filename)
        ...
        #and the coordinate set to the pile of amassed sets
        alignments.append(coords)
    return alignments
